...
function getChosenTraitNames() {
    const names = [];
    startingTraits.forEach(n => { if (n) names.push(n.toUpperCase()); });
    document.querySelectorAll('.trait-slot-row').forEach(row => {
        const n = row.getAttribute('data-chosen') || '';
...
function renderTraitGrid(search) {
    ...
    const chosen = getChosenTraitNames();
    const filtered = TRAITS_DATA.filter(t => {
        if (q && !t.name.toLowerCase().includes(q) && !t.desc.toLowerCase().includes(q)) return false;
        return true;
    });
    // Eligible A-Z first, then ineligible A-Z
    const sorted = [
        ...filtered.filter(t => checkTraitEligible(t)).sort((a,b) => a.name.localeCompare(b.name)),
        ...filtered.filter(t => !checkTraitEligible(t)).sort((a,b) => a.name.localeCompare(b.name))
    ];
    container.innerHTML = sorted.map(t => {
        const alreadyTaken = chosen.includes(t.name.toUpperCase());
        ...
        return `<div class="${cls}" onclick='selectTraitForSlot(${JSON.stringify(t.name)})'>
...
function selectTraitForSlot(traitName) {
    if (_startingTraitSlot !== null) {
        selectStartingTrait(traitName);
        return;
    }
    if (!_traitSlotId) {
        // Called from view-all mode — find first empty trait slot
        const emptySlot = Array.from(document.querySelectorAll('.trait-slot-row')).find(r => !r.getAttribute('data-chosen'));
    ...
    row.querySelector('.trait-slot-name').textContent = traitName;
    row.querySelector('.trait-slot-btn').textContent = 'CHANGE';
    row.querySelector('.trait-slot-clear').style.display = 'inline-block';
    closeTraitModal();
    triggerAutosave();
}
...
/* ===== STARTING TRAITS (overview tab) ===== */
function openStartingTraitModal(slotIdx) {
    _startingTraitSlot = slotIdx;
    _traitSlotId = null;
    document.getElementById('trait-modal').style.display = 'flex';
    document.getElementById('trait-modal-search').value = '';
    renderTraitGrid('');
...
function selectStartingTrait(traitName) {
    if (_startingTraitSlot === null) return;
    startingTraits[_startingTraitSlot] = traitName;
    _startingTraitSlot = null;
    closeTraitModal();
    renderStartingTraitsUI();
    triggerAutosave();
}

function clearStartingTrait(slotIdx) {
    startingTraits[slotIdx] = '';
    renderStartingTraitsUI();
    triggerAutosave();
}

function renderStartingTraitsUI() {
    const container = document.getElementById('ov-traits-inline');
    if (!container) return;
    const maxTraits = mode === 'hc' ? 5 : Infinity;
    const canAdd = startingTraits.length < maxTraits;
    const startHTML = startingTraits.map((name, idx) => {
        if (name) {
            return `<div class="ov-trait-entry" style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
                <span class="ov-trait-dot">★</span>
                <span style="flex:1;font-size:0.78rem;">${name}</span>
                <button onclick="openStartingTraitModal(${idx})" style="font-size:0.55rem;padding:1px 6px;border:1px solid rgba(200,255,212,0.3);background:none;color:#c8ffd4;cursor:pointer;letter-spacing:0.05em;">CHANGE</button>
                <button onclick="removeStartingTrait(${idx})" style="font-size:0.55rem;padding:1px 5px;border:1px solid rgba(255,80,80,0.3);background:none;color:rgba(255,100,100,0.8);cursor:pointer;">✕</button>
            </div>`;
        } else {
            return `<div class="ov-trait-entry" style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
                <span class="ov-trait-dot" style="opacity:0.4;">★</span>
                <span style="opacity:0.4;flex:1;font-size:0.78rem;">EMPTY SLOT</span>
                <button onclick="openStartingTraitModal(${idx})" style="font-size:0.55rem;padding:1px 6px;border:1px solid rgba(200,255,212,0.3);background:none;color:#c8ffd4;cursor:pointer;letter-spacing:0.05em;">SELECT</button>
                <button onclick="removeStartingTrait(${idx})" style="font-size:0.55rem;padding:1px 5px;border:1px solid rgba(255,80,80,0.3);background:none;color:rgba(255,100,100,0.8);cursor:pointer;">✕</button>
            </div>`;
        }
    }).join('');
    const addBtn = canAdd
        ? `<div style="margin-top:4px;"><button onclick="addStartingTraitSlot()" style="font-size:0.58rem;padding:2px 8px;border:1px dashed rgba(200,255,212,0.3);background:none;color:rgba(200,255,212,0.55);cursor:pointer;letter-spacing:0.05em;width:100%;">+ ADD STARTING TRAIT${mode==='hc'?' ('+startingTraits.length+'/5)':''}</button></div>`
        : `<div style="margin-top:4px;font-size:0.58rem;opacity:0.4;letter-spacing:0.05em;text-align:center;">TRAIT LIMIT REACHED (5/5)</div>`;
    const levelHTML = Array.from(document.querySelectorAll('#prog-list .trait-slot-row')).map(r => {
        const n = r.getAttribute('data-chosen') || '';
        return n ? `<div class="ov-trait-entry" style="margin-bottom:3px;"><span class="ov-trait-dot">▸</span><span style="font-size:0.78rem;">${n}</span></div>` : '';
    }).join('');
    container.innerHTML = startHTML + addBtn + (levelHTML ? '<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(200,255,212,0.12);">' + levelHTML + '</div>' : '');
    const ovTEmpty = document.getElementById('ov-traits-empty');
    if (ovTEmpty) ovTEmpty.style.display = 'none';
}

function addStartingTraitSlot() {
    const maxTraits = mode === 'hc' ? 5 : Infinity;
    if (startingTraits.length >= maxTraits) return;
    startingTraits.push('');
    renderStartingTraitsUI();
    triggerAutosave();
}
...
function makeTraitRow(slotId, levelLabel, chosenName) {
    ...
    return `<div class="prog-row trait-slot-row" id="${slotId}" data-chosen="${name}">
...
function getNVImplantLimit() {
    return special.END || 5;
}

function getNVImplantCount() {
    let count = 0;
    IMPLANTS_DATA.forEach(imp => {
        if (imp.cat !== 'bigtmt' && implantsTaken[imp.name]) count++;
    });
    return count;
}
...
function toggleImplant(name) {
    ...
    if (implantsTaken[name]) {
        ...
        // Check limit for NV clinic implants
        if (imp.cat !== 'bigtmt') {
...
function renderImplants() {
    ...
    if (header) header.textContent = `NV CLINIC: ${count} / ${special.END || 5} (END LIMIT)`;
    ...
    const renderGroup = (title, items) => {
        ...
        return `<div class="implant-group">
            ...
            ${items.map(imp => {
                ...
                const canAdd = imp.cat === 'bigtmt' || count < limit || taken;
                const statLabel = imp.cat === 'special' && imp.stat ? ` (+1 ${imp.stat})` : '';
                return `<div class="implant-item ${taken ? 'implant-taken' : ''} ${!canAdd && !taken ? 'implant-locked' : ''}" onclick='toggleImplant(${JSON.stringify(imp.name)})'>
...
function setMode(m, skipSave=false) {
    ...
    if (m === 'hc' && startingTraits.length > 5) startingTraits = startingTraits.slice(0, 5);
...
function buildPerkCard(p) {
    ...
    const addBtnArgs = `${JSON.stringify(p.name)},${JSON.stringify(p.req)},${isIT}`;
    ...
    return `<div class="${cardClass}">
        ...
        <div class="perk-card-actions">
            <button class="action-btn" onclick='addPerkToBuild(${addBtnArgs})'>${addBtnLabel}</button>
        </div>
    </div>`;
}
...
function openLevelUpModal() {
    ...
    const specialSpent = Object.values(special).reduce((a,b)=>a+b,0) - 7;
    const specialReady = specialSpent >= pool;
    const notReadyEl = document.getElementById('lvlup-not-ready');
    if (tagged.size < 3 || !specialReady) {
        ...
        if (!specialReady) {
            notReadyEl.textContent = `⚠ SPEND ALL S.P.E.C.I.A.L. POINTS FIRST (${pool - specialSpent} REMAINING)`;
        } else {
            notReadyEl.textContent = '⚠ TAG 3 SKILLS TO UNLOCK LEVEL UP';
        }
...
function lvlupAdjust(skill, delta) {
    ...
    const mult = isTagged ? 2 : 1;
    const pts = _lvlupSession[skill] || 0;
    if (delta > 0) {
        if (_lvlupPointsLeft <= 0) return;
        const curTotal = skillTotal(skill) + pts * mult;
        if (curTotal + mult > 100) return;
...
function confirmLevelUp() {
    ...
    reCheckAllPerkRows();
    triggerAutosave();
    // Prompt for perk if this level grants one
    const newLvl = charLevel;
    const isPerkLevel = (mode === 'hc') ? (newLvl % 3 === 0) : (newLvl % 2 === 0);
    if (isPerkLevel) {
        setTimeout(() => showPerkLevelUpPrompt(newLvl), 200);
    }
}
...
function showPerkLevelUpPrompt(lvl) {
    ...
    if (!banner) return;
    banner.innerHTML = `<span style="font-size:0.8rem;letter-spacing:1px;">⬆ LVL ${lvl}: PERK SLOT UNLOCKED!</span><button onclick="this.parentElement.style.display='none';jumpToPerkRow(${lvl})" style="margin-left:12px;padding:3px 12px;font-size:0.65rem;background:var(--pip-color);color:black;border:none;cursor:pointer;letter-spacing:1px;font-weight:bold;">→ ASSIGN PERK</button><button onclick="this.parentElement.style.display='none'" style="margin-left:6px;padding:3px 8px;font-size:0.65rem;background:none;border:1px solid var(--pip-color);color:var(--pip-color);cursor:pointer;">LATER</button>`;
    banner.style.display = 'flex';
    // Auto-navigate to prog tab and highlight the new slot
    showTab('prog');
    setTimeout(() => jumpToPerkRow(lvl), 120);
}

function jumpToPerkRow(lvl) {
    showTab('prog');
    setTimeout(() => {
        // Count how many perk levels have occurred up to and including lvl
        let perkIdx = 0;
        for (let i = 2; i <= lvl; i++) {
            if (mode === 'std' ? (i % 2 === 0) : (i % 3 === 0)) perkIdx++;
        }
        const rows = Array.from(document.querySelectorAll('#prog-section-perks .prog-row'));
        const target = rows[perkIdx - 1];
        if (!target) return;
        target.scrollIntoView({ behavior: 'smooth', block: 'center' });
        // Flash highlight
        target.style.transition = 'box-shadow 0.2s';
        target.style.boxShadow = '0 0 0 2px var(--pip-color), 0 0 16px rgba(100,255,100,0.3)';
        setTimeout(() => { target.style.boxShadow = ''; }, 2000);
        const inp = target.querySelector('.prog-name-input');
...
function updateAll() {
    ...
    const traitHTML = [...startingTraits.filter(n=>n).map(n=>`<div class="ov-entry"><span>★ ${n}</span></div>`),
        ...Array.from(document.querySelectorAll('#prog-list .trait-slot-row')).map(r => { const n = r.getAttribute('data-chosen')||''; return n ? `<div class="ov-entry"><span>• ${n}</span></div>` : ''; })
    ].join('') || 'NONE';
    ...
    // Update ov-traits-inline (starting + level-up traits)
    renderStartingTraitsUI();
    ...
    if (lvlBtn) {
        ...
        const pool = (mode === 'hc' ? 30 : 33);
        const specialSpent = Object.values(special).reduce((a,b)=>a+b,0) - 7;
        const ready = tagged.size >= 3 && specialSpent >= pool;
        lvlBtn.classList.toggle('lvlup-btn-ready', ready);
        lvlBtn.classList.toggle('lvlup-btn-locked', !ready);
        lvlBtn.title = !ready
            ? (specialSpent < pool ? `SPEND ${pool - specialSpent} MORE S.P.E.C.I.A.L. PTS` : 'TAG 3 SKILLS FIRST')
            : '';
    }
    ...
    if (skillListEl) {
        skillListEl.innerHTML = skills.map(s => {
            ...
            const breakdown = `BASE:${base}${tagBonus?` TAG:+${tagBonus}`:''}${spent?` LVL:+${spent}`:''}`;
            return `<div class="skill-row${isTagged?' skill-row-tagged':''}" title="${breakdown}">
                ...
                <span class="skill-row-val">${val}</span>
            </div>`;
        }).join('');
    }
}
...
function clearProgRow(btn) {
    ...
    const ni = row.querySelector('.prog-notes-input'); if (ni) ni.value = '';
...
function makeProgRow(levelLabel, isTrait, removable) {
    ...
    const removeBtn = removable
        ? `<button class="prog-remove-btn" onclick="this.closest('.prog-row').remove();updateAll();triggerAutosave();" title="Remove">✕</button>`
        : '';
    return `<div class="prog-row">
        <div class="prog-main-line">
            ...
            <button class="prog-clear-btn" onclick="clearProgRow(this)">✕</button>
            ${removeBtn}
        </div>
        <div class="prog-perk-info">
            ...
            <input type="text" class="prog-notes-input" placeholder="NOTES..." oninput="triggerAutosave()">
        </div>
    </div>`;
}

let _progSections = { perks: true, traits: true };

function toggleProgSection(key) {
    _progSections[key] = !_progSections[key];
    const sec = document.getElementById('prog-section-' + key);
    const btn = document.getElementById('prog-toggle-' + key);
    if (sec) sec.style.display = _progSections[key] ? '' : 'none';
    if (btn) btn.textContent = (key === 'perks' ? 'PERKS ' : 'TRAITS ') + (_progSections[key] ? '▾' : '▸');
}

function filterProgList() {
    const q = (document.getElementById('prog-filter-input')?.value || '').toLowerCase();
    document.querySelectorAll('#prog-list .prog-row').forEach(row => {
        if (!q) { row.style.display = ''; return; }
        const lvlTag = row.querySelector('.lvl-tag')?.textContent?.toLowerCase() || '';
        const nameInput = row.querySelector('.prog-name-input')?.value?.toLowerCase() || '';
        const traitName = row.querySelector('.trait-slot-name')?.textContent?.toLowerCase() || '';
        row.style.display = (lvlTag.includes(q) || nameInput.includes(q) || traitName.includes(q)) ? '' : 'none';
    });
}
...
function renderProgression() {
    ...
    div.innerHTML = `
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid rgba(100,200,120,0.12);">
            <input id="prog-filter-input" class="pip-search" placeholder="FILTER PERKS &amp; TRAITS..." oninput="filterProgList()" style="flex:1;min-width:120px;margin:0;font-size:0.65rem;">
            <button id="prog-toggle-perks" onclick="toggleProgSection('perks')" class="sort-btn active" style="white-space:nowrap;font-size:0.6rem;padding:3px 8px;">PERKS ▾</button>
            <button id="prog-toggle-traits" onclick="toggleProgSection('traits')" class="sort-btn active" style="white-space:nowrap;font-size:0.6rem;padding:3px 8px;">TRAITS ▾</button>
        </div>
        <div id="prog-section-perks"></div>
        <div id="prog-section-traits"></div>`;
    const perkSec = div.querySelector('#prog-section-perks');
    const traitSec = div.querySelector('#prog-section-traits');
    // Level progression rows — perks and level-up traits only (no starting traits here)
    for(let i=2; i<=50; i++) {
        const isP = mode === 'std' ? (i%2===0) : (i%3===0);
        const isT = (i>=5 && (i-5)%4===0);
        if(isP) perkSec.insertAdjacentHTML('beforeend', makeProgRow(`LVL ${i} PERK`, false, false));
        if(isT) {
            const slotId = `trait-slot-lvl-${i}`;
            traitSec.insertAdjacentHTML('beforeend', makeTraitRow(slotId, `LVL ${i} TRAIT`, savedTraits[slotId] || ''));
        }
    }
    // Apply collapse state
    if (!_progSections.perks) { perkSec.style.display = 'none'; document.getElementById('prog-toggle-perks').textContent = 'PERKS ▸'; }
    if (!_progSections.traits) { traitSec.style.display = 'none'; document.getElementById('prog-toggle-traits').textContent = 'TRAITS ▸'; }
}
...
function hydrate(d) {
    ...
    // Restore starting traits (new format) or migrate from old saves
    if (Array.isArray(d.startingTraits)) {
        startingTraits = [d.startingTraits[0]||'', d.startingTraits[1]||''];
    } else {
        startingTraits = ['', ''];
    }
    setMode(d.mode, true);
    ...
    if (d.traits && Array.isArray(d.traits)) {
        // Old saves had starting traits as first 2 entries — migrate them
        let traitArr = d.traits;
        if (!Array.isArray(d.startingTraits) && traitArr.length >= 2) {
            startingTraits = [traitArr[0]||'', traitArr[1]||''];
            traitArr = traitArr.slice(2);
        }
        const traitSlots = Array.from(document.querySelectorAll('#prog-list .trait-slot-row'));
        traitArr.forEach((traitName, i) => {
...
window.onload = () => {
    ...
    if (saved) {
        try {
            ...
            else { setMode('std', true); setOrigin('CW', true); renderStartingTraitsUI(); }
        } catch(e) {
            setMode('std', true); setOrigin('CW', true); renderStartingTraitsUI();
        }
    } else {
        setMode('std', true);
        setOrigin('CW', true);
